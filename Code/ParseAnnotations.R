...
fix_attrs <- function(x) {
  ...
  z <- str_replace_all(y, c("curved|rouded|rounding|rrounded" = "rounded",
...
annotationObj_parseOne <- function(x) { 
  ...
  idx <- which(!names %in% c("x", "y", "xmin", "ymin", 
...
annotationObj_parseAll <- function(x) {
  if (length(x) == 0) { return(data_frame())}
...
polygon_intersect <- function(annot_df) {
  ...
  if (length(tmpintersections) > 0) {
    tmpintersections@data$name <- with(tmpintersections@data, 
                                       paste0(name.1, ", ", name.2))
    tmpintersections@data$name <- sapply(tmpintersections@data$name, 
                                         function(x) 
...
polygon_addfulllabels <- function(annot_df) {
  ...
  if (length(tmpintersections) > 0) {
    tmpintersectdata <- tmpintersectdata %>%
      ...
      mutate(area_int = raster::area(tmpintersections),
